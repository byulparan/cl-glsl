...
(defun process-mesh (ai-mesh scene node node-table)
  (let* ((vertices nil)
	...
	(push (aref position 2) vertices))
      (alexandria:if-let ((position (position-if (lambda (pos) (not (zerop pos))) (ai:components-per-texture-coord ai-mesh))))
	(let* ((tex-coords (aref (aref (ai:texture-coords ai-mesh) position) i)))
