...
(defun update-gpu-stream (environment gpu-stream)
  ...
  (when (/= (%gpu-stream-update-time gpu-stream)
    ...
    (let ((gpu-array (%gpu-stream-array gpu-stream)))
      (if (typep gpu-array 'gl:gl-array)
	  (%gl:buffer-data :array-buffer (gl:gl-array-byte-size gpu-array) (gl::gl-array-pointer gpu-array) :static-draw) 
	(cffi:with-pointer-to-vector-data (pointer gpu-array)
	  (%gl:buffer-data :array-buffer (* (cffi:foreign-type-size (if (typep gpu-array '(simple-array single-float (*)))
									:float :unsigned-char)) (length gpu-array))
			   pointer :static-draw))))
	...
	(cffi:with-pointer-to-vector-data (pointer index-array)
	  (%gl:buffer-data :element-array-buffer (* (cffi:foreign-type-size (if (typep index-array '(simple-array (unsigned-byte 32) (*)))
										:unsigned-int :unsigned-char)) (length index-array))
			   pointer :static-draw)))
